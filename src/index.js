...
form.addEventListener('submit', (event) => {
    ...
    const todoText = input.value; // je recupere le texte dans le zone de saisie du input
    input.value = ''; // on reinitialise la zone de saisie en la vidant

    const newTodo = { text: todoText, done: false, editMode: false }; // on creer une tache
...
const createTodoElement = (todo, index) => {
    ...
    li.addEventListener('click', (event) => {
        todos[index].done = !todos[index].done; // != different de
        displayTodos();
    })
